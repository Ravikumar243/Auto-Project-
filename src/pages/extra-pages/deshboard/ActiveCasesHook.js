...
const ActiveCasesHook = () => {
  ...
  const [filterCancelledCases, setFilterCancelledCases] =
    useState(cancelledCases);
  ...
  const parseCaseDateToIST = (dateStr) => {
    // US format with AM/PM → assume UTC
    if (dateStr.includes("AM") || dateStr.includes("PM")) {
      return dayjs
        .utc(dateStr, ["M/D/YYYY h:mm:ss A", "M/D/YYYY hh:mm:ss A"], false)
        .tz("Asia/Kolkata");
    }

    // DD-MM-YYYY format → already local
    return dayjs(
      dateStr,
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      //     row.caseCreated_Date,
      //     [
      //       "M/D/YYYY hh:mm:ss A", // ✅ Production format
      //       "DD-MM-YYYY HH:mm:ss", // ✅ UAT format
      //       "DD-MM-YYYY", // ✅ fallback
      //     ],
      //     true, // strict parsing
      //   );
      ...
      //   return (
      //     parsedDate.isValid() && parsedDate.format("DD-MM-YYYY") === today
      //   );
      // });

      const filtered = data.filter((row) => {
        const parsedDate = parseCaseDateToIST(row.caseCreated_Date);

        return (
          parsedDate.isValid() &&
          parsedDate.isSame(dayjs().tz("Asia/Kolkata"), "day")
        );
      });
  ...
  useEffect(() => {
    if (search !== "" || todayCases !== "") {
      const result = todayCases.filter(
        (row) =>
  ...
  useEffect(() => {
    ...
    if (recallCancelledSearch !== "") {
      const cancelledCasesList = data
        .filter(
          (row) =>
            row.srN_Remark === "Case Cancelled" ||
            row.srN_Remark === "Service Recalled",
        )
        .filter(
          (row) =>
            row.srN_No
              ?.toLowerCase()
              .includes(recallCancelledSearch.toLowerCase()) ||
            row.customerMobileNo
              ?.toLowerCase()
              .includes(recallCancelledSearch.toLowerCase()),
        );
      ...
      const cancelledCasesList = data.filter(
        (row) =>
          row.srN_Remark === "Case Cancelled" ||
          row.srN_Remark === "Service Recalled",
      );
    ...
    if (comSearch !== "") {
      setCompleted(
        data
          .filter(
            (row) =>
              row.srN_Remark === "Vendor Assigned" &&
              row.rsaStatus === "Case Completed",
          )
          .filter(
            (row) =>
    ...
    if (escSearch !== "") {
      const searchFilteredData = data.filter((row) => {
        ...
        const condition_esc1 =
          row.serviceDrop_IncidentType === "TOWING" &&
          row.vendorName !== "" &&
          row.rsaStatus !== "Case Completed" &&
          row.srN_Remark === "Ongoing" &&
          tat > 7;
        const condition_esc2 =
          row.serviceDrop_IncidentType === "TOWING" &&
          row.srN_Remark === "Ongoing" &&
          row.rsaStatus === "On the way to Incident" &&
          reachedTAT >= 60;
        const condition_esc3 =
          row.serviceDrop_IncidentType === "TOWING" &&
          row.rsaStatus === "Reached at Incident location" &&
          row.srN_Remark === "Ongoing" &&
          reachComTAT >= 60;
        const condition_esc4 =
          row.serviceDrop_IncidentType === "TOWING" &&
          row.rsaStatus === "On the way to drop" &&
          row.srN_Remark === "Ongoing" &&
          drop_TAT >= 60;
        const condition_esc5 =
          row.serviceDrop_IncidentType === "TOWING" &&
          row.rsaStatus !== "Drop Completed" &&
          row.srN_Remark === "Ongoing" &&
          drop_Com_TAT >= 60;
        const condition_esc6 =
          row.serviceDrop_IncidentType === "TOWING" &&
          row.vendorName !== "" &&
          (!row.rsaStatus || row.rsaStatus.trim() === "") &&
          row.srN_Remark === "Ongoing" &&
          (tat > 7 || reachedTAT >= 60);

        const condition_esc8 =
          row.serviceDrop_IncidentType === "RSR" &&
          row.vendorName !== "" &&
          (!row.rsaStatus || row.rsaStatus.trim() === "") &&
          row.rsaStatus !== "Case Completed" &&
          row.srN_Remark === "Ongoing" &&
          tat > 7;
        const condition_esc7 =
          row.serviceDrop_IncidentType === "RSR" &&
          row.rsaStatus === "On the way to Incident" &&
          row.srN_Remark === "Ongoing" &&
          reachedTAT >= 45;
        const condition_esc9 =
          row.serviceDrop_IncidentType === "RSR" &&
          row.rsaStatus === "Reached at Incident location" &&
          row.srN_Remark === "Ongoing" &&
          reachComTAT >= 45;
        ...
        return (
          ((condition_esc1 &&
            ...
            condition_esc8 ||
            condition_esc9) &&
          (row.srN_No?.toLowerCase().includes(escSearch.toLowerCase()) ||
            row.customerMobileNo
              ?.toLowerCase()
              .includes(escSearch.toLowerCase()))
        );
      });
    ...
    if (followsearch !== "") {
      const followFilteredData = data.filter((row) => {
        ...
        const condition2 =
          ...
          row.rsaStatus === "Reached at Incident location" &&
          row.srN_Remark === "Ongoing" &&
          ((tat < 8 && isNaN(reachTAT)) || (tat < 8 && reachTAT <= 60));
        const condition3 =
          row.serviceDrop_IncidentType === "TOWING" &&
          row.rsaStatus === "on the way to incident" &&
          row.srN_Remark === "Ongoing" &&
    ...
    if (mysearch !== "") {
      setMycases(
        data
          .filter((row) => row.assigned_AgentId === agent)
          .filter(
            (row) =>
              row.srN_No?.toLowerCase().includes(mysearch.toLowerCase()) ||
              row.customerMobileNo
                ?.toLowerCase()
    ...
    try {
      ...
      if (formSRN !== "") {
        ...
        if (result.status && result.dataItem && result.dataItem.length > 0) {
          ...
          const summaryData =
            hResult.dataItems && hResult.dataItems.length > 0
              ? hResult.dataItems[0]
              : "";
          const count = 0;
          ...
          if (expandPanel) {
            navigate("/createTickets", {
              state: {
                expandPanel,
                ticketData: data,
                historyStatus: summaryData,
                stepscounts: count,
              },
            });
          } else {
            alert(
              "All required fields are filled. No accordion to open specifically.",
            );
          }
        } else {
          alert("No Data Found!");
