...
const ClientDashboard = () => {
  const { agentClientTotalData } = useContext(VendorFailureContext);
  const { filteredTotals } = useContext(AgentContext);
  ...
  return (
    <Box>
          ...
          {[
            {
              ...
              value: filteredTotals?.onlyCancelled || 0,
              bg: "linear-gradient(135deg, #B0BEC5 0%, #ECEFF1 100%)",
              titleColor: "#37474F",
            },
            {
              title: "Case Denied",
              value: filteredTotals?.caseDenied || 0,
              bg: "linear-gradient(135deg, #EF9A9A 0%, #FFEBEE 100%)",
              titleColor: "#C62828",
            },
            {
              title: "Service Recalled",
              value: filteredTotals?.serviceRecalled || 0,
              bg: "linear-gradient(135deg, #A5D6A7 0%, #E8F5E9 100%)",
              titleColor: "#2E7D32",
            },
          ].map((item, i) => (
            <Card key={i} sx={{ borderRadius: 3, boxShadow: 2 }}>
              <CardContent sx={{ background: item.bg, height: 100 }}>
                <Typography color={item.titleColor} fontWeight={600}>
                  {item.title}
                </Typography>
      ...
      {activeBreakdown === "completed" && (
        ...
        >
          {[
            {
              title: "Cases Completed",
              value: filteredTotals?.onlyCaseCompleted || 0,
              bg: "linear-gradient(135deg, #6FDCC8 0%, #CFF7EF 100%)",
              titleColor: "#0E8F7A",
            },
            {
              title: "Complete Enquiry",
              value: filteredTotals?.completeEnquiry || 0,
              bg: "linear-gradient(135deg, #81D4FA 0%, #E1F5FE 100%)",
              titleColor: "#0277BD",
            },
          ].map((item, i) => (
      ...
      <Box mt={3}>
        {userRole === "client" ? <ParticularClient /> : <ClientTable />}
      </Box>
    </Box>
  );
};
